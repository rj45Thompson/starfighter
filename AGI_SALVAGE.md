...
## Next salvage queue (agent-verified order; items 1-5 of the original queue landed — see ports-completed)
1. combine-iso M8 growth loop (P7) — **needs a `git clone` first (github-only locally)**.
2. P6 ToM: design a falsifiable in-game test (predict another mind's next action from its OWN world, score it) before any claim.
3. Observatory gravity layout — consume GWELL.positions() as an optional KOBS layout mode (direction = relation); the geometry is already computed and proven.
(Path correction recorded: the chainer's repair function is the module-level `repair()` in chainer.py, not a `_gravity_well` symbol — the ledger's earlier name was approximate; the file and the math were exactly where stated.)
