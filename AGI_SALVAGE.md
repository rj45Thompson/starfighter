...
## Next salvage queue (agent-verified order, all paths disk-real; items 1-4 of the original queue landed — see ports-completed)
1. **Gravity-well repair** — `D:\code\Tami\.opus-tools\agi_proto\chainer.py` `_gravity_well` (NOT in TDRE — corrected): pose-graph least-squares, repair-not-prune, candidate for healing weak edges in the live graph.
2. combine-iso M8 growth loop (P7) — **needs a `git clone` first (github-only locally)**.
3. P6 ToM: design a falsifiable in-game test (predict another mind's next action from its OWN world, score it) before any claim.
