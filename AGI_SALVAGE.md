...
## Next salvage queue (agent-verified order; items 1-6 of the original queue landed — see ports-completed)
1. P6 ToM: design a falsifiable in-game test (predict another mind's next action from its OWN world, score it) before any claim.
2. Observatory gravity layout — consume GWELL.positions() as an optional KOBS layout mode (direction = relation); the geometry is already computed and proven.
3. combine-iso follow-ups now the fleet repo is local (`D:\code\combine-iso-agi-fleet`): M9 cooperative-proof (2 provenance-distinct attesters), M10 self-improvement bench — survey before queueing.
