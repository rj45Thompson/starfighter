...
## Next salvage queue (agent-verified order, all paths disk-real; items 1-3 of the original queue landed — see ports-completed)
1. **pillar_router solvers** (P8) — `rootbench_solver.py` SOLVERS: inheritance / constraint / composition / centric, all regex+dict, all high-portability; router abstains on ties (matches the game's ethos).
2. **Gravity-well repair** — `D:\code\Tami\.opus-tools\agi_proto\chainer.py` `_gravity_well` (NOT in TDRE — corrected): pose-graph least-squares, repair-not-prune, candidate for healing weak edges in the live graph.
3. combine-iso M8 growth loop (P7) — **needs a `git clone` first (github-only locally)**.
4. P6 ToM: design a falsifiable in-game test (predict another mind's next action from its OWN world, score it) before any claim.
